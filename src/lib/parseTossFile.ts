...
const INSTITUTION_KEYWORDS = ['거래점', '취급기관', '거래기관', '처리기관', '은행명']
...
function detectColumns(headers: string[]): {
  ...
  return {
    ...
    institutionIdx: find(...INSTITUTION_KEYWORDS),
...
function parseRows(rows: (string | number | undefined)[][]): ParseResult {
  ...
  const { dateIdx, descIdx, merchantIdx, memoIdx, institutionIdx, withdrawIdx, depositIdx, typeIdx, amountIdx } = detectColumns(headers)
